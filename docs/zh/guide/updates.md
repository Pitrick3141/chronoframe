...
## Cloudflare GitHub 自动更新

将已审阅的改动提交并推送到连接仓库的 `main` 分支。Cloudflare Workers Builds 自动运行 `pnpm run build:cloudflare`，通过后执行 `pnpm run deploy:cloudflare`，依次应用远端 D1 迁移和发布 Worker。构建日志与重试都在 Cloudflare 控制台的 Builds 页面查看。

首次连接和权限配置见 [GitHub 集成部署](/zh/guide/getting-started#cloudflare-github-integration)。
